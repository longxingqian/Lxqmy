$(function(){
    ...
    SW.Rpc.BaseProxy.include({
        ZDD_GetRainSumByCollectionCodeAndTimeRange: function (btime,etime,successFn,errorFn) {
            //获得组合站的自由时间段降水
            var params = [];
            params     = params.concat("onehour","chengdu_county",btime,etime);
            return this.invoke('ZDD.GetRainSumByCollectionCodeAndTimeRange', params, 1, successFn, errorFn);
        },
        ...
        zdzProxy.ZDZ_GetDataByStationCodesAndWeatherKeys(code,new Date(),function(res){
            ...
            if(res.length>0){
                $(".temp").text(Math.round(res[0].AIRTEMP_CURRENT_VALUE));
                 console.log(res[0].AIRTEMP_CURRENT_VALUE);
                ...
                //console.log(res[0].RH_CURRENT_VALUE);
                $(".wind").text((res[0].WIND_CURRENT_POWERVALUE||"0")+"级");
